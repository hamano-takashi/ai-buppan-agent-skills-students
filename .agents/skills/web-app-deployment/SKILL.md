...
description: "汎用Webアプリケーションの本番デプロイ・インフラ構築・運用を統括するAgent。 Fly.io / Docker / PostgreSQL / Prisma を中心に、Node.js/Pythonアプリの ゼロからの本番環境構築、TLS設定、DB移行、CI/CD、モニタリングを担当。 フレームワーク非依存の汎用デプロイナレッジ。 「デプロイ」「本番」。"
---
...
| メモリ不足でOOM | ビルド時にメモリ消費が大きい | `関連知識「vm」（この配布版には未同梱。必要なら利用者に確認）` の `memory` を増やす |
