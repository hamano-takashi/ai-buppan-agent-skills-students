---
name: pox-analysis
description: "Analyze positioning with Points of X by identifying POD, POP, and POF from competitor evidence and translating it into the user's product concept, offer design, LP strategy, Ama。"
---
...
- POX is a judgment artifact, so apply PARITY KERNEL: define success, constraints, missing evidence, and hypotheses before the conclusion; trace the causal chain at least three levels deep.
- Produce at least three positioning or concept alternatives and compare expected effect, risk, cost, and difficulty before selecting one. State the tradeoff.
- Separate observed competitor/customer evidence from inference. Claims, figures, rankings, review counts, and superiority language require `fact-check`; legal claims and comparative advertising require `legal-check`.
- If the recommendation becomes platform-specific copy, images, ads, or listings, run `platform-policy-check` using current official sources before finalizing.
